...
volatile int SYM_AUTO=DEF_SYM_AUTO;  //Utilisation du sym�triseur automatique ou non
//volatile int _DETECT;
...
/*
//Calcul du nombre de pas de d�c�l�ration pour passer de la vitesse v � vmin, avec une d�c�l�ration dec.
static unsigned long NbPasDec(unsigned v, unsigned vmin, unsigned dec)
   {
   unsigned long nb;
   nb=MulU(v,v)-MulU(vmin,vmin);
   dec+=dec;
   return nb/dec;
   }   
*/
...
static void CalculConsignes(void)
    {
    static unsigned Vit; //Vitesse de profil trap�zo�dal
    ...
    long vdprec,vgprec; //vitesses pr�c�dentes des roues gauche et droite
...
//    if(SEQ.EtatProfil==P_ARRET) { XDCONS=Xd/MULPREC; XGCONS=Xg/MULPREC; return; }
    if((SEQ.EtatProfil==P_INIT))  //pr�paration et demarrage du mouvement
        {
        POSG=0;
        POSD=0;
        Type=SEQ.Cmde;
        ...
        switch(Type)
           ...
           case CMDE_PDi:
              ...
              vmax=SEQ.PivotVmax;
              SEQ._Pivot=1;
              if(SYM_AUTO && (COULEUR!=COULEUR_DEF)) SEQ._Sens=!SEQ._Sens;
              break;
           case CMDE_ARi:
              SEQ._Sens=1;
           case CMDE_AVi:
        ...
        SEQ._HaltReq=0;
        SEQ._UrgReq=0;
        SEQ.EtatProfil=P_MOVE;              //demarrage d'un mouvement
        }
    if(SEQ.EtatProfil==P_MOVE)  //Mouvement en cours
        {
        //D�but du g�n�rateur de profil de vitesse trap�zo�dal (sur Vit)
        vprec=Vit;
        if(Vit>vmax) vacc=Vit-dec;  //Si vmax a diminu�, on d�c�l�re
        else
           {
           vacc=Vit+acc;   //On essaie d'acc�l�rer ...
           if(vacc>vmax) vacc=vmax; //... et on �cr�te �ventuellement si on va trop vite !
           }
        if(vacc<vmin) vacc=vmin;
        Ndec=_NbPasDec(vacc,vmin,dec); //+((unsigned long)vacc+vprec)/2;  //Calcul du nombre de pas � faire pour s'arr�ter
        if(Ndec<lpp) Vit=vacc; //Si on n'a pas besoin de d�c�l�rer
        else
            {
            if(Vit>vmin+dec) Vit-=dec;
            else Vit=vmin;
            }
         // Fin du g�n�rateur de profil de vitesse trap�zo�dal
         // Vit contient la nouvelle consigne de vitesse du profil

        //Calcul de l'�volution de la position
      ...
      case CMDE_PGi:    //pivot gauche
         ...
         if(SEQ._Pivot) {vd=-CVit; vg=CVit;}
            ...
            if(circ)
               ...
               if(SYM_AUTO && (COULEUR!=COULEUR_DEF)) x=-x;
               vd+=x;
               vg-=x;
               }   
            }
...
static T_Correc CorrecX= { .kp=5.0f,.kd=0.f,.frs=10,.seuilfrs=2, .e_prec=0 };

#if 1 //Petits moteurs Conrad
static T_Correc CorrecG= { .kp=2.f,.kd=20.f,.frs=60,.seuilfrs=8, .e_prec=0 };
static T_Correc CorrecD= { .kp=2.f,.kd=20.f,.frs=60,.seuilfrs=8, .e_prec=0 };
#endif

#if 0
static T_Correc CorrecG= { .kp=0.4f,.kd=10.f,.frs=100,.seuilfrs=8, .e_prec=0 };
static T_Correc CorrecD= { .kp=0.4f,.kd=10.f,.frs=100,.seuilfrs=8, .e_prec=0 };
#endif
...
static struct
   {
   int PXCONS; //Consigne appliqu�e au moteur
   int PX;   // position courante (codeur)
   float nbp; //nombre de pas � faire
   float vmax, vmin, acc;
   float vk,vkp,pc;
   volatile unsigned go:1;
   volatile unsigned done:1;
   unsigned sens:1;  //1 : avant - 0 : arri�re
   unsigned fdec:1; //indique d�c�l�ration enclench�e
   unsigned facc:1;
   }PRM_MOTX=
   {
...
/*
volatile int DBGodovd[1000];
volatile int DBGVD[1000];
volatile int DBGMes=0;
volatile unsigned DBGNb=0;
*/

#if 0
float ErrMax;

static int16_t Correcteur(long cons, long pos, float *e_prec)
{
   const float kp=1.5f; //Gain proportionnel
   const float kd=50.f;  //Action d�riv�e
   const float frs=100;  // Frottement sec (valeur Vitmot pour commencer � tourner en 14V)
   const float seuilfrs=8;
   float e,u,deriv;
   ...
   *e_prec=e;
   ...
   if (u>seuilfrs) u+=frs;   //compensation du frottement sec
   else if(u<-seuilfrs) u-=frs;
