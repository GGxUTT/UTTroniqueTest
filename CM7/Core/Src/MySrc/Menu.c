...
T_DEFPROG PROG_COMPET[]=
   {
      {MatchQualif,"Match QUALIFICATION"},
    ...
    const struct
	   {
    	...
    	uint16_t nbsel;
			   ...
			   {
					   {.txt="Couleur", .aff=AffSelectColor, .sel=&COULEUR, .nbsel=2},
		...
		case K_OK: //OK
		   {
			...
			lcdTextColor565(C565_FerrariRed);
			   ...
			   do
				  ...
				  lcdTextColor565(C565_RedWine);
				  lcdprintf("Vbat=%.1*1000D\t",VBAT);
				  if(VBAT<15000) {lcdTextColor565(C565_BloodRed); lcdprintf(" !!! Faible !!!\t");}
				  else lcdprintf(" Ok !\t");
			      }
			      while(_Tirette);
			   if(MENU[S_MODE].dur)
