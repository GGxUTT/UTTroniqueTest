...
#else
#define DetectAV() USAV()
#define DetectAR() USAR()
#define DetectStop() USStop()
#endif
...
static int LAST_PX,LAST_PY;  //Derni�res positions avant mouvement
...
int SeqEtat(T_POINT *org)
   ...
   if(SEQ._Pivot)
      {
      x|=2;
      if(SYM_AUTO && (COULEUR!=COULEUR_DEF)) x^=1; //
      }
   return x;
   }

//R�cup�ration des flags mis � 0 dans Asserv. pour recopie dans s�quenceur
...
//Ttimeout : num�ro de tempo utilis�e pour Timeout ou -1 si pas de timeout
//Renvoie 0 si arr�ter ou 1 si continuer, et valeur "filtr�e" de _DETECT dans *cus
...
//Renvoie 1 si normal ou 0 si demande de changement de s�quence de profils
static int ExecMovement(void)
   ...
   SEQ.EtatProfil=P_INIT;          //ex�cute le mouvement
   ...
   while((!SEQ.MouvFini)||(SEQ.MouvInterrompu))
      ...
      if(SEQ.MouvFini && SEQ.MouvInterrompu)     //Gestion mouvement interrompu
         {
#if DBG_SEQ
   lcdprintf("Mouvement interrompu [%.1*10Ds]\n",TEMPS_RESTANT);
#endif
         // tester red�marrage possible ???
         if(SEQ._URG)
            {
...
//               DelaiMs(500);
         ...
         //Red�marrage sur m�me profil :
//         static T_CMD cmd2;
//         cmd2=*(SEQ.CURRENT);
//         cmd2.p=SEQ.LRest;
//         SEQ.CURRENT=&cmd2;
         SEQ.Nbp=SEQ.LRest;
//            SEQ.MoveVmax=20000;   //Modification de la vitesse max si n�cessaire
         SEQ.EtatProfil=P_INIT; //Red�marrage
         DelaiMs(2);  //Attente Lecture P_INIT par Asserv
#ifdef __SIM__
      SEQ.MouvFini=0;
#endif
...
extern int X_Cible, Y_Cible; // Coordonn�es du point � atteindre
